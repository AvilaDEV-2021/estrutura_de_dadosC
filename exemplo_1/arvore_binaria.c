...
void Calcula_AlturaNo(TipoArvore **arvore, TipoItem item, int *y)
{
    if((*arvore)->item.chave == item.chave)
    {
        ImprimeArvoreBinaria(arvore,0);
        *y = CalculaAlturaArvoreBinaria(arvore);
       return 0;
    }

    if (!TestaArvoreBinariaVazia((*arvore)->esquerda))
        Calcula_AlturaNo((*arvore)->esquerda, item, y);
    if (!TestaArvoreBinariaVazia((*arvore)->direita))
        Calcula_AlturaNo((*arvore)->direita, item, y);
